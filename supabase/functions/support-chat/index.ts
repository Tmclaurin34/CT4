...
// v7: Local plan is $49/mo. v6: no-size gifts only (no apparel, on purpose); unified sample-brand gallery. v5: brand samples are fictional. v4: B2B/staffing fit.
...
PLANS (monthly): Local $49 — up to 200 customers, 200 texts/mo. Growth $99 — 1,000 customers, 1,000 texts/mo, all integrations, retention analytics. Scale $299 — unlimited customers, 5,000 texts/mo, AI campaign suggestions, white-label (no Clicktide branding on messages). Email sending is included on all plans. Extra texts cost 5 cents from the gift wallet. Mailed postcards are $1.50 each from the gift wallet. 30-day free trial, nothing charged for 30 days, cancel anytime. There's also a free drift assessment: connect read-only and see how many customers have drifted and what they're worth — no card needed.

GIFTS: branded with the business's own logo, printed and shipped on demand — stickers (~$1.58), mug, notebook, postcards pack, crew socks, scented candle, magnet, tote bag, water bottle, tumbler (~$24.50). Gift costs come from a prepaid gift wallet the business tops up. A gift lives on the customer's counter — "nobody throws away a free mug." The website's gift gallery shows the products wearing made-up sample business logos (Café Luna, Iron Tide Fitness, Bloom & Co., Golden Hour, Shore Auto, Pawfect, and others) purely for illustration — not real customers; every real account's gifts print with that business's own uploaded logo, automatically. NO SHIRTS OR HOODIES, on purpose: sizes are a guess when gifts send automatically, and a wrong-size gift says "they don't know me" — every catalog item is one-size (the crew socks are a one-size variant). A business that wants to give apparel can send a Digital Gift Code instead ("come grab your shirt next visit") so the customer picks their own size in store — and that visit is the whole point.
